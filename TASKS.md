...
- [ ] macOS：CI 用 Xcode 工具链从官方源码以 `RUNTIME_PREFIX` 可重定位方式构建，arm64 + x86_64 分别构建后 lipo 合成 Universal；钉死源码 tarball SHA-256
  - 进展（2026-07-08）：`fetch-git-dist` 已接入 macOS source-tarball build plumbing：校验官方 tarball 后按 arm64/x86_64 分别构建安装，再用 `lipo` 合并 Mach-O 文件；fixture 覆盖 prepared source assembly 与 manifest checksum。仍待真实 CI artifact 产出后勾选。
- [ ] Linux：Ubuntu 20.04 容器（glibc 2.31）构建；libcurl/openssl/zlib/pcre2/expat 全静态链入；`NO_GETTEXT/NO_PERL/NO_TCLTK` 裁剪；glibc 动态链接（不用 musl）
  - 进展（2026-07-08）：`fetch-git-dist` 已接入 Linux source-tarball build plumbing：非 Ubuntu 20.04 Linux host 通过 Docker `ubuntu:20.04` 构建，Ubuntu 20.04 内可直接构建，并在安装后拒绝动态链接 libcurl/openssl/zlib/pcre2/expat。远端 build run `28914517678` 已完成 Linux 编译但被安装后动态第三方库审计拦截；本轮改为用 `pkg-config --static` 生成第三方静态库链接参数，拆出 `-ldl`/`-pthread` 等系统库保持 glibc 动态链接，并覆盖 Git Makefile 的 `EXTLIBS`/`CURL_LDFLAGS`/`EXPAT_LIBEXPAT`/OpenSSL link 变量；后续 run 暴露 Ubuntu 20.04 libcurl 的 Kerberos 传递依赖无静态库，已将 `gssapi_krb5`/`krb5`/`k5crypto`/`com_err` 归入动态系统认证依赖，仍继续审计需求列出的 libcurl/openssl/zlib/pcre2/expat 不能动态链接。仍待真实 CI artifact 产出后勾选。
...
- [ ] 关窗保护：点 X / `Cmd/Ctrl+W` 时若有写操作进行中或未完成冲突/审查模式 → 确认框「有操作正在进行，关闭将取消该操作并恢复到操作前状态」；确认执行标准取消/恢复后关闭；`Cmd/Ctrl+Q` 逐窗口执行同样检查
  - 进展备注（2026-07-08，close guard evidence v2）：新增 `pnpm phase9:close-guard:audit` 静态审计，将写入口分成 cancellable、wait-only、以及不应由 close guard 取消三类并在 CI 可跑脚本里固定：cancellable 覆盖 clone/sync/accept remote history/review/branch/stash/commit/revert 等 19 个 IPC 写入口，逐项断言请求带 `operationId`、后端注册 cancel token、Tauri 发 cancellable progress、前端生成 operation id；wait-only 明确为 `open_repository`（可能更新子模块但无 cancel token）和 `restore_changes`（短事务恢复写入），其中 open repository 已接入 StartScreen wait-only close guard，CloseRequested 与 pending quit 组件测试覆盖不会关闭/不会误取消 clone。WDIO real-git full-chain spec 新增冲突 overlay 期间 `Cmd/Ctrl+W`/`Ctrl+W` close guard 断言，并纳入 `pnpm e2e:check` 静态 gate，避免回退到纯 Git 链路。当前本地仍无真实 `ARTISTIC_GIT_DIST_DIR` + tauri-driver 的 Linux/Windows WDIO 实跑证据，暂不勾选。
- [ ] 崩溃隔离三层验证：WebView 崩溃 → 主进程检测自动重载该窗口 + 崩溃弹窗；React 错误边界仅影响自身窗口；Rust 命令 Result 化全量复查 + panic hook → 崩溃弹窗端到端接通
  - 进展备注：已补 renderer crash 注入重载 + pending crash 弹窗通路、Rust panic `crash-reported` → 前端崩溃弹窗桥接与测试；Tauri 2.11.5 的公开 `WebviewEvent` 仍只有 DragDrop，但 Builder 暴露 macOS/iOS-only `on_web_content_process_terminate` hook，本阶段已将该 hook 接到现有 renderer crash reload/pending crash 弹窗路径。Windows/Linux 仍无可证明的 native WebView crash 自动检测公共 hook，因此完整崩溃隔离验收仍未关闭。`Cmd/Ctrl+W` 菜单关闭与窗口 CloseRequested 共用 close guard 判定，active backend operation 也会注册 guard，冲突/审查模式关窗恢复与退出取消已有覆盖；已补 `operation-progress` 的 repository/window 归属字段与前端过滤，避免多窗口操作误触发其它窗口 close guard；9C close guard 已抽出共用确认/恢复/关闭 flow，clone cancel、stash restore recovery cancel、conflict cancel、review exit/recovery 均纳入该 flow，`Cmd/Ctrl+Q` pending exit 逐窗口复用同一 gate；已新增后端通用 `cancel_operation` 注册表与 IPC 命令，clone 已迁移到通用 token 注册，RepositoryShell 对 `cancellable=true` 的 active backend operation 会尝试通用取消后再关闭，新增验证覆盖 `cancel_operation` 返回失败时不关闭并取消 pending quit；`cancellable=false` 或未注册 token 的 active backend operation 仍阻止关闭并提示等待，不假装可取消。此前已让 `syncCurrentBranch`/`syncBranch`/`syncAllBranches`/`acceptRemoteHistory`/`createBranch`/`checkoutBranch`/`startReviewMode` 由前端生成 operation id，Tauri 发可取消进度，后端注册 token，git 子进程在取消时被 kill。本轮进一步补齐 commit/revert、review sync/exit/recovery、delete branch/delete safety backup、create/restore/delete stash 的 operation id、Tauri cancellable progress、后端 token 注册与取消上下文；commit 本地阶段取消后走 rollback point 恢复 index/HEAD，revert 本地阶段取消后 `revert --abort`/reset/auto-stash restore，stash restore 失败或取消后用 recovery stash 明确恢复。新增 RepositoryShell/HistoryWorkbench 断言覆盖前端 operation id，git-runner/app registry 测试覆盖 token kill/注册；真实 commit/revert 子进程 kill 恢复测试已补但当前无 `ARTISTIC_GIT_DIST_DIR` 时按现有策略跳过运行体。仍需全量真实 Git/E2E 证据与剩余 wait-only 写路径审计，暂不勾选关窗保护。
