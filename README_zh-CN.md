...
release workflow 会在 `main` push 与 `workflow_dispatch` 运行，但只有来源为
`main` 且 `ENABLE_MAIN_RELEASE=true` 时才发布，不使用 GitHub Environment 或人工
审批。闸门未开启时，只执行测试与 Tauri `--no-bundle` dry-run 构建，不发布产物。
手动运行可以使用自动版本计算，也可以覆盖 SemVer 升级级别。每个平台发布 job 都会
恢复精确的仓库工具链 cache，在本平台执行 ensure 与 verify，打包固定资源树，并校验
包内 manifest 与文件哈希；发布不再消费其他 workflow 的二进制 artifact。仓库和
GitHub Releases 必须保持公开，以保证 updater 产物 URL 可访问。
