...
- 每阶段的**完成定义（DoD）**：
  ...
  2. 三平台 CI（lint + 单元 + Rust 集成测试）全绿；核心 Git 流程测试全部使用**真实临时仓库与内嵌 git**，从 `src-tauri/resources/git-dist` 固定资源树取显式路径，禁止 fake/mock 命令且绝不回退系统 git；
...
- [x] Git 测试 bootstrap 契约：标准测试命令自动确保固定 dev git-dist；缺失或版本不符时测试失败，禁止使用系统 git
...
- [x] Windows：下载官方 MinGit 预编译包 + 官方 Win32-OpenSSH 发行包，校验发布页二进制 SHA-256
  ...
  - 历史记录（2026-07-09）：曾存在仅下载部分来源并允许受阻目标跳过的证据模式；该模式现已删除。当前任一目标的来源、构建或验证失败都会令 ensure 与审计 workflow 失败。
  ...
  - 流程变更（2026-07-10）：删除每日上游版本扫描。当前 Win32-OpenSSH pin 随人工工具链 revision 固定，不查询或自动选择新版本；维护者决定更新时再修改版本、URL、SHA-256 并生成新 lock。
  ...
  - 流程变更（2026-07-11）：当前 `10.0.0.0p2-Preview` 作为 revision `2026.07.11.1` 的明确固定输入；动态 release 查询脚本已删除，是否切换 stable 只在人工更新 revision 时评估。
...
- [x] Linux：Ubuntu 20.04 容器（glibc 2.31）构建；libcurl/openssl/zlib/pcre2/expat 全静态链入；`NO_GETTEXT/NO_TCLTK` 裁剪，保留 Perl-backed porcelain（如 `git submodule`）；glibc 动态链接（不用 musl）
  ...
  - 历史验证（2026-07-08）：旧 Git Distribution workflow 的 macOS/Linux reusable artifact 曾通过 runtime smoke。该跨 workflow artifact 流程现已由仓库本地 cache + ensure/verify 取代。
...
- [x] 本地工具链命令：`pnpm git-toolchain:ensure` 在缺失时下载/构建并原子激活到固定 dev resources，有效时只校验复用；二进制产物不提交普通 Git 仓库
  - 流程变更（2026-07-10）：公开入口收敛为 ensure、verify、config check 与人工 update。资源树固定为 `src-tauri/resources/git-dist`，cache 固定为 `.cache/artistic-git/git-toolchain`，不再导出路径或接受输出目录参数。
  - 流程变更（2026-07-10）：资源目录整体 ignored，不再保留受版本控制的 mount README，因此能够以完整目录原子交换。
  - 流程变更（2026-07-10）：Git/LFS/Windows OpenSSH 受人工 `toolchain_revision` 和 lock 约束；helper 按源码、`Cargo.lock` 与固定 Rust 版本的独立指纹复用。
  - 完成（2026-07-10）：README 双语示例与开发入口均已切换到 ensure/verify；旧 fetch/check 公共命令和路径输出参数已移除。
  - 完成（2026-07-10）：固定资源树由 manifest schema 2、完整 SHA-256、版本与 runtime smoke 验证；三平台均可在冷 cache 下生成完整 dev resources。
- [x] CI cache 复用策略：每个测试/打包 job 恢复精确指纹 cache，cache miss 时按 lock 与配置现场下载/构建并校验；不跨 workflow 传递完整工具链
  - 历史记录（2026-07-08）：旧 workflow 的缓存恢复、跨 workflow artifact 与 placeholder 分支已废弃。当前 CI 每个 job 使用精确 lock/config/helper/contracts key 恢复仓库工具链 cache，并无条件执行 ensure/verify；审计 workflow 则冷构建三平台且只上传 manifest、构建证据与尺寸报告。
  - 完成（2026-07-10）：Git Toolchain Audit workflow 对三平台执行冷构建与 runtime smoke，仅上传 manifest、build evidence 和 size report，不上传完整分发。
  - 完成（2026-07-10）：CI 与 Release cache key 使用 target + lock/config/helper/contracts/Cargo 输入的精确 hash，不配置前缀恢复；ensure 对恢复内容进行最终验证。
- [x] 版本升级流程文档化：升级 = 修改配置文件走 PR + 全量测试 + 记入更新日志

**验收**：CI 三平台 ensure/verify 通过且校验和匹配；本地脚本产物可执行 `git --version` / `git lfs version`；后续 job 通过精确 cache 复用产物；篡改校验和时构建失败。
...
**验收**：Rust 集成测试（真实临时仓库 + 内嵌 git）：写锁互斥与忙时拒绝、取消后状态恢复、进度解析、环境隔离（系统/全局 config 注入不生效）、固定工具链缺失时硬失败且不回退系统 git、自检失败路径。
...
- [x] 自动检查：静默后台下载（静默仅指不主动弹窗）；`checking` / `available` / `downloading` / `notAvailable` / `failed` 均与手动检查一致，在目标窗口的设置-关于页展示；仅 `ready` / `releaseAvailable` 弹更新提示框（新版本号 + Release notes 更新日志 + 「立即重启更新」/「稍后」→ 下次启动自动安装）；自动检查失败不主动弹窗，仍按小时重试
- [x] 手动检查：立即显示「发现新版本」弹窗 + 下载进度，其余状态同样在设置-关于页展示；检查失败提示「检查失败」
...
- [x] E2E 全链路：克隆 → 修改 → 提交 → 同事推送 → 同步 → 冲突 → 解决 → 撤回（真实临时远程仓库）
  ...
  - 历史记录（2026-07-08）：新增 `e2e/tauri/full-chain-real-git.e2e.ts`，使用真实内嵌工具链创建临时 bare remote，并通过 Tauri WebView 点击克隆入口/填写克隆表单；此后 E2E 已收敛为始终运行真实工具链，不再需要 opt-in 开关。
  ...
  - 流程变更（2026-07-10）：Linux/Windows CI 在 E2E 前恢复精确工具链 cache 并执行 ensure/verify，随后无条件执行 full-chain。资源缺失或损坏直接失败，不再产出 availability skipped 成功。
  ...
  - 流程变更（2026-07-10）：删除外部 Git Distribution run 输入和 artifact 下载。E2E report 记录当前固定资源树的 target、revision、fingerprint、Git/LFS SHA-256 与 runtime smoke；任一证据缺失即失败。
  ...
  - 历史验证（2026-07-08）：真实 Git 2.55 暴露并验证了 checkout auto-stash cancel、sync 失败恢复、submodule gitlink 提交、auto-tracking stale ref/prune 与测试夹具 fast-forward 修复。当前复测入口统一为先执行 toolchain ensure 的标准测试命令。
...
- [x] 失败注入全矩阵复查：每个融合操作（同步/自动跟踪/提交/撤回/审查/切换/子模块提交）每一步注入失败，断言恢复到操作前状态 + 「工作区与 HEAD 干净可用」全局不变量
  ...
  - 历史验证（2026-07-08）：真实 Git 2.55 复测补齐矩阵实现缺口；当前通过固定资源树与标准 `pnpm cargo:test` 入口完成同类 workspace 全量验证。
...
- [x] 性能验证：万级提交历史滚动/几万文件大树 status 时延（fsmonitor 生效断言）/巨型二进制 + LFS 仓库操作
  - 进展备注（2026-07-08）：新增 `pnpm phase12:perf`，使用固定内嵌工具链验证历史分页 200/页、启用 fsmonitor/untracked cache 的大文件树 status 时延，以及巨型二进制经 git-lfs 以 pointer 入库且工作区保留真实内容。
  ...
  - 流程变更（2026-07-10）：性能测试缺少固定内嵌工具链时直接失败，不再以 skipped report 作为成功结果；CI 仍上传可审计报告。
  - 流程变更（2026-07-10）：性能 evidence 记录 Node/OS/CPU/内存、CI 元数据、profile 阈值、manifest revision/fingerprint、Git/LFS 版本和 SHA-256；可执行文件必须位于固定资源树。真实内嵌工具链已成为唯一模式，保留 `--heavy` 作为规模选择，不保留缺失工具链旁路。
  - 验证备注（2026-07-08）：指向 `/usr/bin/git` 的伪分发会被 realpath gate 拒绝，不能作为 light profile pass；当前固定资源树缺失时同样硬失败。
  - 历史验证（2026-07-08）：真实 macOS universal 工具链下，10000 commits 分页、50000 untracked files status 与 128MB LFS 场景通过；当前重型入口为 `pnpm phase12:perf:heavy`，会先自动 ensure。
  - 流程变更（2026-07-10）：CI test job 不再接收外部工具链 workflow 输入，也不下载完整分发 artifact；各平台恢复精确 cache 后执行 ensure/verify，再运行性能测试。cache miss 会现场构建，工具链缺失或验证失败直接令 job 失败。
  ...
  - 流程变更（2026-07-10）：Phase 12 summary 直接验证当前固定工具链 manifest、fingerprint、SHA-256 与 runtime smoke；缺失或失败返回非零，不再依赖独立 build workflow 的 source/blocker evidence。
  ...
  - 历史验证（2026-07-09）：Linux/macOS heavy perf 达到 artifact-backed pass；旧 Windows skipped 结论已不再是可接受状态，当前三平台 CI 必须各自 ensure 成功并产生真实 pass evidence。
...
- [x] `0.1.0` 正式发布演练：走完整 Release 流程 + 三平台安装冒烟 + 自动更新升级到 `0.1.1` 演练
  ...
  - 流程变更（2026-07-10）：正式 release package job 在本平台恢复精确 cache 并执行 ensure/verify，不再下载或验证另一 workflow 的完整工具链 artifact。打包后继续解析 packaged manifest 并校验每个文件 SHA-256。
  - 流程变更（2026-07-10）：删除旧 build-evidence verifier；Release 静态契约改为验证固定 cache、ensure/verify、包前 runtime smoke 与包后资源校验。
